...
pub async fn app(pool: PgPool) -> Router {
	...
	.route("/fetch_apod", get(handlers::fetch_and_store_apod))
        .route("/protected", get(handlers::protected))
	//.route("/apod/:apod_id/upvote", post(handlers::upvote))
        //.route("/apod/:apod_id/downvote", post(handlers::downvote))
	.route("/apod_list", get(handlers::list_all_apods))
